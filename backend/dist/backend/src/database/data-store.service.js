...
let DataStoreService = class DataStoreService {
    ...
    saveGameAttempt(attemptDto, userId) {
    ...
    async persistAttemptToSupabase(id, attemptDto, xpEarned, relationEvents, targetUserId) {
        if (!this.pgPool)
            return;
    ...
    async findOrCreateGoogleUser(payload) {
        if (this.isSupabaseConnected && this.pgPool) {
            try {
                const checkRes = await this.pgPool.query(`SELECT * FROM users WHERE google_id = $1 OR email = $2 LIMIT 1`, [payload.googleId, payload.email]);
                if (checkRes.rows.length > 0) {
                    ...
                    const updateRes = await this.pgPool.query(`UPDATE users 
             ...
             RETURNING *`, [
                    ...
                    const insertRes = await this.pgPool.query(`INSERT INTO users 
             ...
             RETURNING *`, [
                    ...
                    await this.pgPool.query(`INSERT INTO user_profiles (user_id, preferred_language, target_wpm, daily_goal_minutes)
             VALUES ($1, 'vi', 450, 15)
             ON CONFLICT (user_id) DO NOTHING`, [newUser.id]).catch(() => { });
                    return newUser;
                }
            }
            catch (err) {
    ...
    async syncUserProfile(userId, data) {
        if (this.isSupabaseConnected && this.pgPool) {
            try {
                const res = await this.pgPool.query(`UPDATE users
           SET total_xp = GREATEST(total_xp, $1),
               ...
               updated_at = NOW()
           WHERE id = $5
           RETURNING *`, [data.xp, data.level, data.streak, data.bestStreak || data.streak, userId]);
                if (res.rows.length > 0)
                    return res.rows[0];
            }
            catch (err) {

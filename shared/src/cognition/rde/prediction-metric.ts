...
export interface IPredictionEvaluation {
  isCorrect: boolean;
  predictionError: number;     // 0.0 (chính xác tuyệt đối) đến 1.0 (sai số tối đa)
  brierCalibrationScore: number; // Thước đo hiệu chuẩn tự tin (càng gần 0 càng tốt)
  feedbackVi: string;
}
...
export function evaluatePrediction(
  ...
  answer: IRdePredictionAnswer,
  _resultingState?: IRdeBoardState
 ...
 */
export function calculateCrossDomainTransfer(
  sourceAccuracy: number,
  targetAccuracy: number,
  sourceLatencyMs: number,
  targetLatencyMs: number
): { transferRatio: number; isBridgeSuccessful: boolean } {
  if (sourceAccuracy <= 0) return { transferRatio: 0, isBridgeSuccessful: false };
